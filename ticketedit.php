...
if(isset($_GET['post'])){
	...
	if(isset($_POST['title']) && isset($_POST['post']) && trim(htmlspecialchars($_POST['title'])) != '' && trim(strip_tags($_POST['post'],$center['html_tag']),"&nbsp;") != '') {
		$SQL->query("UPDATE ticket SET post_title = '%s', post = '%s' WHERE id = '%d' AND posted = '%s'",array(
			htmlspecialchars($_POST['title']),
			sc_xss_filter($_POST['post']),
			$_GET['id'],
			$_SESSION['Center_Username']
		));
		header("Location: ticketview.php?id=".$post_row['id']);
	}
	...
	$reply = $SQL->query("SELECT * FROM ticket_reply WHERE id = '%d' AND posted='%s'",array($_GET['id'],$_SESSION['Center_Username']));
	...
	$SQL->query("UPDATE ticket_reply SET reply = '%s' WHERE id = '%d' AND posted = '%s'",array(
		sc_xss_filter($_POST['reply']),
		$_GET['id'],
		$_SESSION['Center_Username']
	));
	header("Location: ticketview.php?id=".$reply_row['post']);
	}
...
<h2>Edit Reply</h2>
<form action="ticketedit.php?reply&id=<?php echo $_GET['id']; ?>" method="POST" name="form1">
	<div class="controls">
...
<h2>Edit Post</h2>
<form action="ticketedit.php?post&id=<?php echo $_GET['id']; ?>" method="POST" name="form1">
