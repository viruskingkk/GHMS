...
$post = $SQL->query("SELECT * FROM ticket WHERE id = '%s'",array($_GET['id']));
...
$_status = $SQL->query("SELECT * FROM `ticket_status` WHERE `id`='%d'",array(abs(intval($post_row['status']))))->fetch_assoc();

if(isset($_GET['page'])){
	$limit_start = abs(intval(($_GET['page']-1)*20));
	$reply_sql = sprintf("SELECT * FROM ticket_reply WHERE post = '%s' ORDER BY id ASC LIMIT %d,20",$_GET['id'],$limit_start);
} else {
	$limit_start = 0;
	$reply_sql = sprintf("SELECT * FROM ticket_reply WHERE post = '%s' ORDER BY id ASC LIMIT %d,20",$_GET['id'],$limit_start);
}
...
if((isset($_GET['reply']))&& isset($_POST['reply']) && trim($_POST['reply'],"&nbsp;") != ''){
	$SQL->query("INSERT INTO ticket_reply ( post,reply, ptime, posted) VALUES ('%s','%s','%s','%s')",array(
		$_GET['id'],
	...
	if($_SESSION['Center_Username']!=$post_row['posted']){
		 sc_addnotice(
			"ticketview.php?replying&id=".$_GET['id'],
			$_SESSION['Center_Username']."在您的帖子中發表回覆",
			$_SESSION['Center_Username'],
			$post_row['posted']
		);
	}
	header("Location: ticketview.php?replying&id=".$_GET['id']);
}
...
<?php }elseif(isset($_GET['statusmoving'])){ ?>
...
<form action="ticketview.php?id=<?php echo $_GET['id']; ?>&reply" method="POST" name="form1">
	<div class="controls">
...
<?php } else { ?>
<ul class="breadcrumb">
	<li><a href="ticket.php">Support Center</a><span class="divider">/</span></li>
	<li><a href="ticket.php?fid=<?php echo $_status['statusname']; ?>"><?php echo $_status['statusname']; ?></a><span class="divider">/</span></li>
	<li class="active"><?php echo lt_replace($post_row['post_title']); ?></li>
</ul>
<h2>
  <?php echo lt_replace($post_row['post_title']); ?>&nbsp;<a href="ticketview.php?id=<?php echo $post_row['id']; ?>&reply" class="btn btn-primary btn-mini">New Reply</a></h2>
<div id="1" class="post">
	<ul class="inline">
		...
		<li>
			<a href="ticketedit.php?post&id=<?php echo $post_row['id']; ?>" class="btn btn-info btn-small">Edit</a>
			<a href="ticketmove.php?id=<?php echo $post_row['id']; ?>" class="btn btn-warning btn-small">Move</a>
...
<div id="<?php echo $reply_floor; ?>" class="post">
	<ul class="inline">
		...
		<li><a href="ticketedit.php?reply&id=<?php echo $reply_row['id']; ?>" class="btn btn-info btn-small">Edit</a> 
...
$nav = $SQL->query("SELECT * FROM ticket_reply WHERE post = '%s'",array($_GET['id']));
...
if($pageTotal>1){
	...
	for($i=1;$i<=$pageTotal;$i++){
		if(@$_GET['page']!=$i){
				echo '<li><a href="ticketview.php?id='.$_GET['id'].'&page='.$i.'">'.$i.'</a></li>';
			}else{
